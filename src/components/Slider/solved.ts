...
export const Slider = ({
  ...
  const showTimestamp = isOnSlider || isDragging;
  ...
  return (
    <div className="relative">
      <div
        className={cn(
          "absolute left-0 ml-[20px] rounded-md bg-[#3399FF] px-2 py-1.5 text-center text-xs text-white transition-opacity duration-200",
          !showTimestamp && "opacity-0"
        )}
        style={{
          bottom: "calc(100% + 12px)",  // equivalent to sideOffset
          pointerEvents: "none"
        }}
      >
        <div>
      ...
      <div
        ...
        onMouseLeave={() => setIsOnSlider(false)}
      >
